...
// Generate quote email for the project
async function generateQuoteEmail(
  ...
  try {
    // Fetch selections with product info
    const { data: selections } = await supabaseAdmin
      .from('project_selections')
      .select(`
        *,
    ...
    if (selections && selections.length > 0) {
      for (const selection of selections) {
        const price = selection.product_price_snapshot || 0
        totalAmount += price * (selection.quantity || 1)
        selectionLines.push(
          `<tr>
            <td style="padding: 8px; border-bottom: 1px solid #eee;">${selection.product_name_snapshot}</td>
            <td style="padding: 8px; border-bottom: 1px solid #eee; text-align: center;">${selection.quantity || 1}</td>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
    <h1 style="color: white; margin: 0; font-size: 24px;">${showroomName}</h1>
    <p style="color: rgba(255,255,255,0.9); margin: 10px 0 0 0;">Quote for Your Kitchen Project</p>
  </div>

  <div style="background: #f9fafb; padding: 30px; border: 1px solid #e5e7eb; border-top: none;">
    <p style="margin-top: 0;">Dear ${customerName},</p>

    <p>Thank you for visiting our showroom and using our room scanning technology. Below is your personalized quote based on the products you selected.</p>

    <div style="background: white; border-radius: 8px; padding: 20px; margin: 20px 0; border: 1px solid #e5e7eb;">
      <p style="margin: 0 0 10px 0; font-size: 14px; color: #6b7280;">Reference Number</p>
      <p style="margin: 0; font-size: 20px; font-weight: bold; font-family: monospace;">${referenceNumber}</p>
    </div>

    ${selections && selections.length > 0 ? `
    <h3 style="margin-bottom: 15px;">Selected Products</h3>
    ...
    <div style="background: #fef3c7; border: 1px solid #f59e0b; border-radius: 8px; padding: 20px; margin: 20px 0; text-align: center;">
      <p style="margin: 0; color: #92400e;">No products were selected during your visit. Contact us to discuss your project requirements.</p>
    </div>
    `}

    <div style="background: #10b981; color: white; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
      <p style="margin: 0 0 5px 0; font-size: 14px; opacity: 0.9;">Estimated Total</p>
      <p style="margin: 0; font-size: 32px; font-weight: bold;">${grandTotal}</p>
    </div>

    <p style="font-size: 14px; color: #6b7280;">This quote is valid for 30 days. Prices are estimates and may vary based on final measurements and installation requirements.</p>

    <p>If you have any questions or would like to proceed with your project, please don't hesitate to contact us.</p>

    <p style="margin-bottom: 0;">Best regards,<br><strong>${showroomName}</strong></p>
  </div>

  <div style="text-align: center; padding: 20px; color: #9ca3af; font-size: 12px;">
    <p style="margin: 0;">This quote was automatically generated by CabinetScan</p>
  </div>
</body>
</html>`

    const bodyText = `
${showroomName} - Quote for Your Kitchen Project

Dear ${customerName},

Thank you for visiting our showroom and using our room scanning technology.

Reference Number: ${referenceNumber}

${selections && selections.length > 0
  ? `Selected Products:\n${selections.map(s => `- ${s.product_name_snapshot}: $${(s.product_price_snapshot || 0).toFixed(2)}`).join('\n')}`
  : 'No products were selected during your visit. Contact us to discuss your project requirements.'}
...
`
    ...
    const { error: insertError } = await supabaseAdmin
      ...
      .insert({
        ...
        subject: `Your Kitchen Quote from ${showroomName} - Ref: ${referenceNumber}`,
        body_html: bodyHtml,
        body_text: bodyText,
        ...
        quote_summary: {
          total: grandTotal,
          products_count: selections?.length || 0
        },
        status: 'pending'
      })

    if (insertError) {
      console.error('[QUOTE EMAIL] Error creating quote email:', insertError)
    } else {
      console.log(`[QUOTE EMAIL] ✅ Quote email generated for project ${projectId}`)
    }
  } catch (err) {
    console.error('[QUOTE EMAIL] Error generating quote email:', err)
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    // Generate quote email (async, don't block)
    generateQuoteEmail(
      project.id,
      ...
      console.error('[QUOTE EMAIL] Failed to generate quote email:', err)
    })
