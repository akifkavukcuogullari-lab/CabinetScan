// Vapi outbound call utility for Voice Agent
...
/**
 * Initiate an outbound phone call via the Vapi API.
 *
 * Reads credentials from platform_settings:
 *   - vapi_api_key
 *   - vapi_phone_number_id
 *
 * The webhook URL for tool calls and end-of-call events is derived from
...
export async function initiateVapiCall(options: InitiateCallOptions): Promise<InitiateCallResult> {
  ...
  try {
    // 1. Read Vapi credentials from platform_settings
    ...
    // 2. Build the webhook server URL
